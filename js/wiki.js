...
export function renderWiki(wiki, itemId) {
  ...
  if (wiki.cos_e) {
    ...
    sections.push(block(i18n.t('wiki.section.what'), `<p>${escapeHtml(cosE)}</p>`));
  }
  if (wiki.perche) {
    const percheKey = `wiki.${itemId}.perche`;
    const perche = i18n.t(percheKey, wiki.perche);
    sections.push(block(i18n.t('wiki.section.why'), `<p>${escapeHtml(perche)}</p>`));
  }
  ...
  if (wiki.tempo_stima) {
    ...
    sections.push(
      `<p class="text-sm text-zinc-500 mt-4 flex items-center gap-1.5"><svg class="w-4 h-4 text-amber-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> ${i18n.t('wiki.section.time')} <span class="text-amber-400/90">${escapeHtml(tempoStima)}</span></p>`
    );
  }
