...
async function init() {
  ...
  function renderProgress() {
    ...
    const categoryLabels = {
      tecnica: i18n.t('stats.technique', CATEGORIA_LABEL.tecnica),
      trucco: i18n.t('stats.trick', CATEGORIA_LABEL.trucco),
      cardistry: i18n.t('stats.cardistry', CATEGORIA_LABEL.cardistry),
    };
    ...
    root.innerHTML = `
      <div class="grid md:grid-cols-3 gap-4 mb-10">
        <div class="rounded-xl border border-amber-500/30 bg-amber-500/5 p-6 text-center shadow-lg">
          ...
          <p class="text-sm text-zinc-400 mt-1">${i18n.t('progress.completion')}</p>
        </div>
        <div class="rounded-xl glow-card p-6 text-center">
          <p class="text-4xl font-bold text-zinc-100 font-serif">${stats.done}</p>
          <p class="text-sm text-zinc-400 mt-1">${i18n.t('progress.items-learned')}</p>
        </div>
        <div class="rounded-xl glow-card p-6 text-center">
          <p class="text-4xl font-bold text-zinc-100 font-serif">${stats.total - stats.done}</p>
          <p class="text-sm text-zinc-400 mt-1">${i18n.t('progress.still-to-learn')}</p>
        </div>
      </div>
      ${Object.entries(byCat)
        .map(([cat, list]) => {
          if (!list.length)
            return `<section class="mb-8"><h2 class="text-lg font-bold text-zinc-200 mb-2 font-serif">${categoryLabels[cat]}</h2><p class="text-zinc-500 text-sm">${i18n.t('progress.no-items')}</p></section>`;
          return `
          <section class="mb-8">
