  ...
  root.innerHTML = `
    <div class="grid md:grid-cols-3 gap-4 mb-10">
      <div class="rounded-xl border border-amber-500/30 bg-amber-500/5 p-6 text-center shadow-lg">
        ...
        <p class="text-sm text-zinc-400 mt-1">Completamento globale</p>
      </div>
      <div class="rounded-xl glow-card p-6 text-center">
        <p class="text-4xl font-bold text-zinc-100 font-serif">${stats.done}</p>
        <p class="text-sm text-zinc-400 mt-1">Voci imparate</p>
      </div>
      <div class="rounded-xl glow-card p-6 text-center">
        <p class="text-4xl font-bold text-zinc-100 font-serif">${stats.total - stats.done}</p>
        <p class="text-sm text-zinc-400 mt-1">Ancora da imparare</p>
      </div>
    </div>
    ${Object.entries(byCat)
      .map(([cat, list]) => {
        if (!list.length)
          return `<section class="mb-8"><h2 class="text-lg font-bold text-zinc-200 mb-2 font-serif">${CATEGORIA_LABEL[cat]}</h2><p class="text-zinc-500 text-sm">Nessuna voce ancora completata in questa categoria.</p></section>`;
        return `
        <section class="mb-8">
          <h2 class="text-lg font-bold text-zinc-200 mb-3 font-serif border-b border-zinc-800 pb-1.5">${CATEGORIA_LABEL[cat]} (${list.length})</h2>
